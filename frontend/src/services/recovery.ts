...
/**
 * Generate `count` random recovery codes and wrap the user's RSA private
 * key with each one. Returns the raw codes (to display once) and the
 * server payload (to upload).
 */
...
export const uploadRecoveryCodes = async (payload: CodePayload[]) => {
 ...
 * private key + an exported public key (for sanity / new master rewrap).
 */
...
export const completeRecovery = async (params: {
  ...
  // After recovery, always upgrade to argon2id (best-practice default).
  ...
  // We don't have access to the user's vault keys (those were wrapped with
  // the OLD master). After recovery, vaults will release to recipients on
  // schedule but the user can no longer view them in the dashboard.
...
// Re-export utilities so the recovery page can use them through one import
